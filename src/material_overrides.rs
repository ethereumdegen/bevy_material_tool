...
pub fn material_overrides_plugin(app: &mut App) {
    app.init_state::<MaterialOverridesLoadingState>()
        .add_systems(
            OnEnter(MaterialOverridesLoadingState::Extracting),
            load_material_overrides,
        )
        .add_systems(
            OnEnter(MaterialOverridesLoadingState::Building),
...
pub fn begin_loading_materials(mut next_state: ResMut<NextState<MaterialOverridesLoadingState>>) {
    ...
    for evt in asset_ready_event.read() {
        match evt {
            AssetEvent::LoadedWithDependencies { id } => {
                if material_overrides_resource
                    .doodad_materials_gltf
                    .as_ref()
                    .is_some_and(|h| h.id() == *id)
                {
                    let Some(doodad_materials_gltf) = gltf_assets.get(*id) else {
                        continue;
                    };
                    ...
                    for (material_name, material_handle) in &doodad_materials_gltf.named_materials {
                        ...
                        material_overrides_resource
                            .extracted_materials_map
                            .insert(material_name.to_string(), material_handle.clone());
                    }
...
fn build_material_overrides(
    ...
    for (built_material_name, material_config) in material_types_config.material_types.iter() {
        if let Some(extracted_material_handle) = extracted_materials
            .get(&material_config.material_name)
            .clone()
        {
            let Some(extracted_material) = material_assets.get(extracted_material_handle) else {
                continue;
            };
            ...
            let built_material = match material_config
                .shader_type
                .clone()
                .unwrap_or(MaterialShaderType::StandardMaterial)
            {
                MaterialShaderType::StandardMaterial => {
                    ...
                    OverrideMaterialHandle::Standard(asset_server.add(cloned_base_material))
                }
                MaterialShaderType::FoliageMaterial => {
                    ...
                    OverrideMaterialHandle::Foliage(asset_server.add(new_foliage_material))
                }
            };
            ...
            built_materials_resource
                .built_materials_map
                .insert(built_material_name.clone(), built_material);
        }
    }
...
fn handle_material_overrides(
    ...
    material_override_query: Query<
        (Entity, &MaterialOverrideComponent),
        Or<(
            Changed<MaterialOverrideComponent>,
            Added<RefreshMaterialOverride>,
        )>,
    >,
    ...
    for (mat_override_entity, mat_override_request) in material_override_query.iter() {
        ...
        let extracted_material = built_materials_resource.find_material_by_name(&material_name);
                ...
                match new_material_handle {
                    ...
                    OverrideMaterialHandle::Foliage(mat_handle) => {
                        commands
                            ...
                            .try_insert(MeshMaterial3d::<FoliageMaterialExtension>(
                                mat_handle.clone(),
                            ));
                    }
                }
